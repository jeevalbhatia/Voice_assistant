#Text to speach conversion Library
import pyttsx3
import datetime
...
def takecommand():
    #Input from laptop ka microphone return string output
    r=sr.Recognizer()
    with sr.Microphone() as source:
        print("Friday is Listening BE CAREFULL!!!!")
        # r.pause_threshold =1 ye gap batata hain bolne main if this is 0 jaise hi kuch bolne ke baad thoda bhi gap liya input le lega
        r.pause_threshold =1
...
# smtplib used less secured app in gamil --> google search
def sendEmail(to, content):
    server = smtplib.SMTP('smtp.gmail.com', 587)
    server.ehlo()
    server.starttls()
    server.login('jeeval2703@gmail.com', 'mbhw ijsb grzg ktsa')
    server.sendmail('jeeval2703@gmail.com', to, content)
    server.close()
...
# Windows ke kisi bhi software ya installed cheezo ko kholne ke liye use os 
#Hum Gareeb log hain mac ki setting nahi pata khud dhoond lena
        elif 'time' in query:
            strTime = datetime.datetime.now().strftime("%H:%M:%S")
            speak(f"The time is {strTime}")
            ...
            try:
                ...
                content = takecommand()
                speak("Who do you want to send E-mail to")
                print("Please Type The Email ID")
                to=input()
                sendEmail(to,content)
                speak("Email has been sent")
            except Exception as e:
                speak("Sorry, unable to send E-mail")
